...
exports.listIncidents = functions.https.onRequest(async (request, response) => {
    console.log("test");
    const snapshot = await admin.firestore().collection('incidents').where('incident.status', '==', 'open').get();
    const query = snapshot.docs[0].data();
    let responseText = `You have no open incidents :tada:`;
    if (query) {
        responseText = `Open Incidents' tinyIds: ${query.incident.tinyId}`;
    }
    ...
    try {
        ...
        attachments.forEach((i, idx, value) => {
            if (idx !== 0) {
                ...
                console.log(`checklist item: ${firebase_key} status: ${!query.status[firebase_key].status}`);
